    ...
    if game.state == "action":
        for i,act in enumerate(game.actions):
                button:
                    ...
                    if id2pos(game.premoving_who.y)-40*(len(game.actions))>0:
                        ypos id2pos(game.premoving_who.y)-40*(i+1)
                    else:
                        ypos id2pos(game.premoving_who.y)-40*(i+1 - len(game.actions) ) #pour eviter que ça sorte de l'écran
                    text act["text"] style "style_action" size 30
                    background Solid( "#000000" )
                    hover_background "#00a"
                    
                    # if len(act)==2:
                    #     action [Function(updateVision), SetVariable("game.premoving_who.AP", 0),  Jump(act["label"])]
                    # if len(act)==3:
                    action [Function(game.updateVision), SetVariable("game.premoving_who.AP", 0), Call(act["label"],act["variables"])]
